...
class Comment(models.Model):
    ...
    # === 新增的情感分析字段 ===
    sentiment_score = models.FloatField(verbose_name="情感分数", default=0.5)
    ...
    def save(self, *args, **kwargs):
        # 只有在创建新评论或内容发生变化时才进行情感分析
        if self.content and (not self.pk or self._state.adding):
            from snownlp import SnowNLP
            try:
                s = SnowNLP(self.content)
                self.sentiment_score = s.sentiments

                # 根据分数设置情感标签
                if self.sentiment_score > 0.6:
                    self.sentiment_label = "正面"
                elif self.sentiment_score < 0.4:
                    self.sentiment_label = "负面"
                else:
