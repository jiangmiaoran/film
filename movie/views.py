...
# 在 views.py 末尾添加这个函数
def update_existing_comments_sentiment(request):
    """为现有评论批量添加情感分析（可选）"""
    if request.user.is_superuser:  # 只有管理员可以执行
        comments = Comment.objects.filter(sentiment_score=0.5)
        updated_count = 0
        ...
        return HttpResponse(f"成功更新 {updated_count} 条评论的情感分析")
